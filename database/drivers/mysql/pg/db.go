package pg // import "gnorm.org/gnorm/database/drivers/mysql/pg"
...
func (y *YesOrNo) Scan(src interface{}) error {
	...
	s, ok := src.([]byte)
	if !ok {
		return errors.Errorf("expected YesOrNo to be a []byte, but was %T", src)
	}
	switch string(s) {
	case "YES":
		*y = true
	...
	case "NO":
		*y = false
		return nil
	default:
		return errors.New("unexpected value for YesOrNo: " + string(s))
	}
}
